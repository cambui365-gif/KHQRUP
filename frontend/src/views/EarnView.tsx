...
import { Currency, Language, SavingsPlan, UserSavingsRecord, View } from '../types';
...
export const EarnView: React.FC<EarnViewProps> = ({ lang, setView }) => {
  ...
  const loadSavingsData = async () => {
    ...
    if (plansRes.success) setSavingsPlans(plansRes.data || []);
    if (savingsRes.success) setUserSavings(savingsRes.data || []);
  };
  ...
  const calculateInterest = () => {
    ...
    if (!ic.isEnabled || balance < ic.minBalanceToEarn) return { interest: 0, apy: 0 };
    let apy = 0;
    ...
    const interest = Math.min((balance * (apy / 100)) / 365, ic.dailyPayoutCap);
    return { interest, apy };
  };
  ...
  const handleSubscribe = async () => {
      ...
      const res = await userApi.subscribeToPlan(selectedPlan.id, amount, autoRenew);
