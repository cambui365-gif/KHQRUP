...
export const KHQRView: React.FC<KHQRViewProps> = ({ lang, setView }) => {
  const { user, config: systemConfig, setDepositModalOpen, setUserPin, pay, refreshData } = useAuth();
  ...
  const [scannedData, setScannedData] = useState<ParsedQRLocal | null>(null);
  ...
  const handleScanSuccess = async (decodedText: string) => {
    ...
    // Use backend to parse QR
    const res = await paymentApi.parseQR(decodedText);
    if (!res.success || !res.data) {
      stopCamera();
      alert(t.invalidKHQR);
      setCameraVersion(prev => prev + 1);
      return;
    }
    ...
    const data: ParsedQRLocal = {
      merchantName: parsed.merchantName || 'Unknown',
      merchantCity: parsed.merchantCity,
      amount: parsed.amount,
      currency: parsed.currency === 'KHR' ? Currency.KHR : parsed.currency === 'VND' ? Currency.KHR : Currency.USD,
      bankName: parsed.bankCode || 'KHQR',
      raw: decodedText,
      isValid: true,
      isVietQR: parsed.type === 'VIETQR',
      merchantId: parsed.accountNumber,
      bankCode: parsed.bankCode,
      isFixedAmount: parsed.amount > 0,
    };

    if (data.isVietQR) {
      alert(t.vietqrMaintenance);
      setCameraVersion(prev => prev + 1);
      return;
    }

    setScannedData(data);
    if (data.amount && data.isFixedAmount) {
      setInputAmount(formatNumber(data.amount.toString()));
  ...
  const conversionRate = systemConfig?.exchangeRates[billCurrency] || 1;
  ...
  const handleStartPayment = () => {
    ...
    if (!user?.hasPin && !user?.pinHash) { setStep('CREATE_PIN'); setPin(''); setNewPin(''); } else { setStep('PIN'); setPin(''); }
  };
  ...
  const handleConfirmNewPin = async () => {
    ...
    await setUserPin(pin); alert(t.pinSetSuccess); setStep('PIN'); setPin('');
  };
  const handleDepositRedirect = () => { setDepositModalOpen(true); setView(View.HOME); };

  const handlePaymentSubmit = async () => {
    setStep('PROCESSING');

    const paymentPromise = pay(
      finalAmount, billCurrency, pin,
      scannedData?.merchantName, scannedData?.raw,
      scannedData?.bankName, userNote,
      scannedData?.merchantCity, scannedData?.merchantId
    );
    ...
    if (result.success && result.transaction) {
      ...
      if (result.isWrongPin) { setStep('PIN'); setPin(''); setIsShaking(true); setTimeout(() => setIsShaking(false), 500); alert(`Wrong PIN. Remaining: ${result.remainingAttempts}`); }
      else { alert(result.error || "Failed"); setStep('PIN'); setPin(''); }
    }
  };
