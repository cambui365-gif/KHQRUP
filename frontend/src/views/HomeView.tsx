...
export const HomeView: React.FC<HomeViewProps> = ({ lang, setView }) => {
  ...
  const usdRate = systemConfig?.exchangeRates[Currency.USD] || 1.0;
  const khrEquivalent = usdtBalance * khrRate;
  const usdEquivalent = usdtBalance * usdRate;
  const userWalletAddress = user?.customDepositAddress || userWalletAddr || "Loading...";
  ...
  const handleQuickClaim = async (e: React.MouseEvent) => {
    ...
    setIsClaiming(true);
    try { await claimInterest(); } catch (err) { console.error(err); }
  ...
  const handleSubmitDeposit = async () => {
    ...
    const success = await requestDeposit(rawVal);
    setIsSubmitting(false);
    if (success) { alert(t.depositSent); setDepositModalOpen(false); setDepositAmount(''); }
  };
